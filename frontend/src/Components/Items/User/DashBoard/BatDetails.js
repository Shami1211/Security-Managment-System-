...
function BatDetails() {
    ...
    const fetchItems = async () => {
      try {
        ...
        const batItems = response.data.items.filter(item => item.category.toLowerCase() === 'bat');
        setItems(batItems);
        setNoResults(batItems.length === 0);
      } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="userhomebk">
        <p className="topic_para">"Acquire Your Premium Cricket<br></br> Equipment Today"</p>
