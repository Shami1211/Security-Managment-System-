...
function HelmetsDetails() {
  ...
  const fetchItems = async () => {
    try {
      ...
      const helmetsItems = response.data.items.filter(
        (item) => item.category.toLowerCase() === "helmets"
      );
      setItems(helmetsItems);
      setNoResults(helmetsItems.length === 0);
    } catch (error) {
